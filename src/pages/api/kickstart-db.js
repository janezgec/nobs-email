...
export async function POST({ request }) {
  ...
  try {
    ...
    await createCollection(pb, userId, database.id, 'news', [
      ...
      { name: 'title', type: 'text', description: 'News title' },
      { name: 'link', type: 'url', description: 'Link to the news article' },
      { name: 'summary', type: 'text', description: 'News summary' }
    ], 'Collect all news from emails, no matter how small.');

    await createCollection(pb, userId, database.id, 'people_mentions', [
      { name: 'name', type: 'text', description: 'Name of the person mentioned' },
      { name: 'company', type: 'text', description: 'Company where they work if available' },
      { name: 'link', type: 'text', description: 'Link associated' },
      { name: 'context', type: 'text', description: 'Context in which the person was mentioned' }
    ...
    await createCollection(pb, userId, database.id, 'images', [
      ...
      { name: 'description', type: 'text', description: 'Description of the image based on the context' }
